// 서버 주문 전송 API 설정 (로컬 테스트용 localhost 사용 권장)
/* ============================================================
   API / 인증 설정
   - 프론트엔드는 정적 파일로 동작하고, 주문/회원/로봇 상태는 Flask 서버 API에 위임합니다.
   - 토큰은 localStorage에 보관해 새로고침 후에도 로그인 상태를 복구합니다.
   ============================================================ */
const API_BASE_URL = "http://192.168.10.92:5000";
const AUTH_TOKEN_KEY = "robocaseAuthToken";
// 로봇이 어느 정도 진행된 뒤에는 물리 작업 안전성 때문에 취소를 막습니다.
const CANCEL_PROGRESS_LIMIT = 31;

function getAuthToken() {
    // API 요청마다 현재 저장된 Bearer 토큰을 읽습니다.
    return localStorage.getItem(AUTH_TOKEN_KEY) || "";
}

function setAuthToken(token) {
    // 로그인 성공 시 저장, 로그아웃/토큰 만료 시 제거를 한 함수로 처리합니다.
    if (token) {
...
async function apiFetch(path, options = {}) {
    // 모든 서버 호출의 공통 래퍼입니다. Authorization/JSON 헤더를 자동으로 붙입니다.
...
async function uploadOrderToServer(dataUrl, orderInfo) {
    // 결제 직후 호출되는 주문 업로드 단계입니다.
    // 서버에는 미리보기 이미지와, 직접 그린 선만 있는 경우 로봇 경로용 strokeData를 함께 보냅니다.
    app.showToast("서버로 주문 정보를 전송 중입니다...");
    console.log("uploadOrderToServer - Starting, API_BASE_URL:", API_BASE_URL);

    try {
        // 이미지 업로드/템플릿처럼 raster가 섞이면 로봇 경로 변환이 어렵기 때문에 strokeData를 보내지 않습니다.
        const hasStrokeData =
            store.canvas &&
            Array.isArray(store.canvas.strokes) &&
            store.canvas.strokes.length > 0 &&
            !store.canvas.containsRasterContent;
        ...
        const payload = {
            ...
            imageBase64: dataUrl,
            strokeData: hasStrokeData ? {
                canvasWidth: app.canvasManager.canvas ? app.canvasManager.canvas.width : null,
                canvasHeight: app.canvasManager.canvas ? app.canvasManager.canvas.height : null,
                strokes: store.canvas.strokes
            } : null
        };

        console.log("uploadOrderToServer - Payload created, size:", JSON.stringify(payload).length);
        console.log("uploadOrderToServer - Sending to:", `${API_BASE_URL}/api/orders`);
...
const DESIGN_TEMPLATES = [
    // 현재는 기본 Ghost 템플릿만 노출합니다. 템플릿을 늘릴 때는 여기에 추가하면 됩니다.
    { id: "ghost", label: "Ghost", src: GHOST_TEMPLATE_SVG }
];

const refreshIcons = () => {
    // View는 innerHTML로 다시 그려지므로 렌더링 후 Feather 아이콘 치환을 매번 호출해야 합니다.
...
const imageSrcToDataUrl = (src) => new Promise((resolve, reject) => {
    // 외부/템플릿 이미지를 서버 전송 가능한 data URL로 바꾸는 유틸입니다.
...
const store = {
    /* 앱 전체의 단일 상태 저장소입니다.
       별도 프레임워크 없이 route를 다시 렌더링하기 때문에,
       선택값/로그인/캔버스/주문 상태를 이 객체에 모아 둡니다. */
...
const TopNav = (showBack = true) => `
    <!-- 모든 주요 화면에서 재사용되는 상단바입니다. 로그인 상태면 회원정보 버튼을 함께 보여줍니다. -->
...
const Views = {
    /* 화면 템플릿 모음입니다.
       app.render(route)가 route 이름에 따라 아래 함수 중 하나를 호출해 #app 안에 삽입합니다. */
    Main: () => `
...
const app = {
    /* 앱 컨트롤러입니다.
       화면 이동, 서버 요청, 결제/취소/상태 폴링, 캔버스 매니저 초기화를 이 객체가 조율합니다. */
    container: document.getElementById('app'),

    async init() {
        // 앱 최초 부팅: 토스트 DOM 생성, 세션 주문 복구, 브라우저 뒤로가기 핸들러, 로그인 복구를 처리합니다.
    ...
    async restoreAuth() {
        // 저장된 토큰이 있으면 /me API로 유효성을 확인해 사용자 정보를 store에 채웁니다.
        if (!getAuthToken()) {
    ...
    navigate(route, replace = false) {
        // 로그인 보호가 필요한 route는 미인증 상태에서 login으로 강제 이동합니다.
        if (
    ...
    render(route, shouldScroll = true) {
        // 현재 route에 맞는 템플릿을 생성해 #app에 삽입하고, 화면별 후처리를 실행합니다.
    ...
    toggleAccordion(brand) {
        if (store.activeAccordion === brand) store.activeAccordion = null; // ?リ린
        else store.activeAccordion = brand; // ?닿린
        this.render('orderStep1', false); // ?ㅽ겕濡?諛⑹?
    },
    ...
    async submitAuth() {
        // 로그인/회원가입 폼을 같은 함수에서 처리합니다. authMode에 따라 API path와 payload가 달라집니다.
    ...
    async saveProfile() {
        // 회원정보 화면에서 수정한 배송/연락처 정보를 서버에 저장하고 store.auth.user를 갱신합니다.
    ...
    async logout() {
        // 서버 로그아웃은 실패해도 클라이언트 토큰은 제거해 다음 요청이 인증 없이 나가게 합니다.
        try {
    ...
    saveCanvasAndNext() {
        // 투명 캔버스를 흰 배경 PNG로 합성해 결제/상태 화면에서 깨지지 않는 미리보기를 만듭니다.
        ...
        try {
            ...
            // Fallback: try to keep flow moving even if canvas export is blocked.
            try {
        ...
        if (!previewDataUrl) {
            // Minimal valid placeholder image so checkout can still render.
    ...
    async processPayment() {
        // 결제 버튼 클릭 이후의 핵심 흐름: 이미지 확보 -> 서버 주문 업로드 -> 제작 상태 화면 진입.
        ...
        // designDataUrl이 없으면 캔버스에서 직접 생성
        if (!imagePayload) {
            try {
    ...
    openCancelConfirm() {
        // 취소 가능 진행률인지 먼저 확인하고, 가능할 때만 확인 모달을 띄웁니다.
    ...
    async cancelCurrentOrder() {
        // 현재 주문 ID를 기준으로 서버에 취소 요청을 보내고 상태 화면 문구를 즉시 갱신합니다.
    ...
    showImpactStopModal(message, reason = '') {
        // 로봇 외부 충격 감지 시 사용자가 놓치지 않도록 상태 화면 위에 안내 모달을 한 번 띄웁니다.
    ...
    async simulateStatusProcess() {
        // 이름은 simulate지만 실제로는 서버의 내 주문 목록과 로봇 상태 API를 1초마다 폴링합니다.
            ...
            try {
                ...
                if (myOrder) {
                    ...
                    const impactDetected =
                        // 서버 주문 상태 또는 로봇 stage 중 하나라도 충격 중단이면 중단 UI로 전환합니다.
                    ...
                    // 洹몃젮吏??꾩븞 ?대?吏 ?쒖떆 (諛깆뿏??寃쎈줈??留욎떠 /uploads/ 異붽?)
                ...
                console.error("?곹깭 ?뺤씤 ?ㅽ뙣:", e);
            }

            // ?꾩쭅 done???꾨땲硫?1珥??ㅼ뿉 ?ㅼ떆 ?뺤씤
    ...
    // Canvas 愿由?媛앹껜 (Undo 湲곕뒫 異붽?)
    canvasManager: {
        /* 캔버스 전담 관리자입니다.
           DOM canvas 초기화, 그림 입력, 이미지/템플릿 배치, undo, 확장 편집을 모두 여기서 처리합니다. */
        canvas: null,
        ctx: null,

        init() {
            // CSS 크기보다 2배 큰 실제 캔버스를 만들어 고해상도 화면에서도 선이 덜 흐리게 보이도록 합니다.
        ...
        saveState() {
            // 캔버스 이미지 스냅샷을 저장해 undo가 이전 상태로 돌아갈 수 있게 합니다.
            // ?꾩옱 罹붾쾭???곗씠?곕? 諛곗뿴?????(Undo??
        ...
        undo() {
            // 마지막 스냅샷을 버리고 직전 스냅샷 이미지를 다시 그립니다.
            if (store.canvas.history.length <= 1) {
                app.showToast("더 이상 되돌릴 수 없습니다.");
                return;
            }
            store.canvas.history.pop();
            if (!store.canvas.containsRasterContent && store.canvas.strokes.length > 0) {
        ...
        loadImage(event) {
            // 사용자가 업로드한 이미지는 자유 선 데이터가 아니므로 raster 콘텐츠로 표시하고 strokeData 전송을 중단합니다.
        ...
        drawImageInSafeArea(img, shouldClear = true) {
            // 이미지/템플릿은 로봇이 실제로 그릴 수 있는 안전 영역 안에 비율 유지로 중앙 배치합니다.
            const rect = this.canvas.parentElement.getBoundingClientRect();
            const safeArea = {
                x: rect.width * 0.11,
                y: rect.height * 0.18,
        ...
        loadTemplateImage(src, label) {
            // 사전 템플릿을 캔버스에 배치하고, 결제 미리보기용 designDataUrl도 즉시 갱신합니다.
            store.canvas.strokes = [];
            store.canvas.currentStroke = null;
            store.canvas.containsRasterContent = true;
        ...
        getSafeAreaRect() {
            // CSS의 .robot-safe-area와 동일한 비율입니다. 확장 편집/더블클릭 판정도 이 좌표를 사용합니다.
            const rect = this.canvas.parentElement.getBoundingClientRect();
            return {
                x: rect.width * 0.11,
                y: rect.height * 0.18,
        ...
        openExpandedEditor() {
            // 작은 폰 프레임 대신 안전 영역만 크게 띄워 그리고, 완료 시 원본 캔버스의 안전 영역에 되돌립니다.
        ...
        bindEvents() {
            // 마우스와 터치 이벤트를 모두 같은 좌표계로 변환해 데스크톱/모바일 드로잉을 지원합니다.
            const getPos = (e) => {
            ...
            const startDraw = (e) => {
                // 선을 새로 시작하며, raster 콘텐츠가 없는 경우 서버 전송용 stroke 포인트도 함께 기록합니다.
                e.preventDefault();
            ...
            const draw = (e) => {
                // 화면 좌표로는 즉시 선을 그리고, 실제 canvas pixel 좌표로는 로봇용 포인트를 누적합니다.
                e.preventDefault();
            ...
            const stopDraw = () => {
                // 한 획이 끝나면 history와 strokes에 확정합니다.
        ...
        clear() {
            // 캔버스와 관련 주문 미리보기/템플릿/선 데이터를 모두 초기 상태로 되돌립니다.
            const rect = this.canvas.parentElement.getBoundingClientRect();
            this.ctx.clearRect(0, 0, rect.width, rect.height);
